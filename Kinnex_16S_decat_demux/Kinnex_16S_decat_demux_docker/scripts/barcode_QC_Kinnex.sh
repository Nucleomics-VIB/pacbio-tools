...
while getopts "r:i:o:m:p:s:f:h" opt; do
  case $opt in
    r) opt_rmdfile=${OPTARG} ;;
    i) opt_infile=${OPTARG} ;;
    o) opt_outfolder=${OPTARG} ;;
    m) opt_mincnt=${OPTARG} ;;
...
if [ ! -d "${opt_outfolder}" ]; then
    echo
    echo "${opt_outfolder} folder not found!"
    exit 1
fi
...
cmd="Rscript --vanilla -e 'rmarkdown::render(
  ...
  output_dir=\"${opt_outfolder}\",
  intermediates_dir = file.path(\"${opt_outfolder}/tmp\"),
