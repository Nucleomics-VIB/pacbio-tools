...
infolder="/data/NC_projects/4586_ThessaVanPee_PacBio/merged_runs/fastq_reads"
barcode_file="/data/NC_projects/4586_ThessaVanPee_PacBio/merged_runs/Exp4586_merged_SMRTlink_barcodefile.csv"

# destination folder for the nextflow outputs
outfolder="/data/NC_projects/4586_ThessaVanPee_PacBio/merged_runs/pb-16S-nf_10k"
...
grp=$(echo "${fq}" | grep -o -E 'run_[1-4]{1}')
echo -e "${pfx}\t${grp}\t${bc}"
...
TMPDIR="${outfolder}/tmp" nextflow run main.nf \
  ...
  --colorby "${colorby}" \
  -profile docker \
  -resume 2>&1 | tee ${outfolder}/run_log_resume.txt
