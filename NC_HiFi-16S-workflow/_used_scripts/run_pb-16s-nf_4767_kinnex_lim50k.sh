...
barcode_file="${basefolder}/ToSend/Exp4767_SMRTLink_Barcodefile.csv"

# destination folder for the nextflow outputs
outfolder="${basefolder}/pb-16S-nf_kinnex_50k"
...
(echo -e "sample_name\tcondition\tsmplid\tsmplgrp\tgrpid";
for fq in ${infolder}/*.fastq.gz; do
...
echo -e "${bc}\t${label}\t${smplid}\t${smplgrp}\t${grpid}"
