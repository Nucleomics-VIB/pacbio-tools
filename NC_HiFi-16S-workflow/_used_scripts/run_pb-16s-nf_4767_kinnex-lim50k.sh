...
basefolder="/data/NC_projects/4767_KDePaepe_PacBio"
infolder="${basefolder}/ToSend/fastq_results"
barcode_file="${basefolder}/ToSend/Exp4767_SMRTLink_Barcodefile_num.csv"

# destination folder for the nextflow outputs
outfolder="${basefolder}/pb-16S-nf_kinnex-50k"
...
# subsample 50000 reads for large samples
subsample=50000
...
(echo -e "sample_name\tcondition\tlabel\tsmplgrp\tgrpid";
for fq in ${infolder}/*.fastq.gz; do
bc=$(basename "$fq" | cut -d "." -f 1)
# smplid	smplgrp	grpid
smplid=$(cat "${barcode_file}" | grep "${bc}" | cut -d "," -f 2 | tr -d '\r')
label=$(cat "${barcode_file}" | grep "${bc}" | cut -d "," -f 3)
smplgrp=$(cat "${barcode_file}" | grep "${bc}" | cut -d "," -f 4 | tr -d '\r')
grpid=$(cat "${barcode_file}" | grep "${bc}" | cut -d "," -f 5 | tr -d '\r')
echo -e "${bc}\t${smplid}\t${label}\t${smplgrp}\t${grpid}"
