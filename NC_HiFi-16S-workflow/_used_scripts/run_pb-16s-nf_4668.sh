...
barcode_file="${basefolder}/Exp4668_SMRTLink_Barcodefile.csv"

# destination folder for the nextflow outputs
outfolder="${basefolder}/pb-16S-nf_10k"
...
(echo -e "sample_name\tcondition\tlabel\tgenotype";
for fq in ${infolder}/*.fastq.gz; do
bc=$(basename "$fq" | cut -d "." -f 1)
label=$(cat "${barcode_file}" | grep "${bc}" | cut -d "," -f 2)
grp=$(cat "${barcode_file}" | grep "${bc}" | cut -d "," -f 3 | tr -d '\r')
gen=$(cat "${barcode_file}" | grep "${bc}" | cut -d "," -f 4 | tr -d '\r')
echo -e "${bc}\t${grp}\t${label}\t${gen}"
