...
# -r <run_id (obtained from "gsutil ls gs://gc-to-nucleomicscore/<run_name>")>
# -s <smart-cell_id (obtained from "gsutil ls gs://gc-to-nucleomicscore/<run_name><run_id>")>
...
while getopts "R:r:s:lh" opt; do
  case $opt in
    ...
    l) echo "# Runs data currently available on the bucket";
    		gsutil ls gs://gc-to-nucleomicscore/${rundir:-""}/${runid:-""};
			exit 0 ;;
...
# get run folder
echo -e "\n# getting run data"
gsutil -m rsync "gs://gc-to-nucleomicscore/${rundir}/${runid}/${scid}/" .
