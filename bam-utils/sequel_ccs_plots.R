...
# usage: sequel_ccs_plots.R <output of ccsbam2passdist.sh on CCS bam>
#
# Stephane Plaisance VIB-NC November-19-2020 v1.0
...
option_list = list(
  ...
  make_option(c("-m", "--minlength"), type="numeric", default="0", 
              help="minimal CCS length [default= %default]", metavar="numeric"),
  make_option(c("-M", "--maxlength"), type="numeric", default="1000000", 
              help="maximal CCS length [default= %default]", metavar="numeric"),
  make_option(c("-a", "--minaccuracy"), type="numeric", default="0", 
              help="minimal CCS accuracy [default= %default]", metavar="numeric")
); 
...
pdf(gsub("_npass-dist.txt", "_plots.pdf", basename(opt$infile)), width = 10, height = 10)
